...
| **無前綴** | 全部 | 預設從 **Asset Bundle (YooAsset)** 加載資源。 | `SettingWindow` |
...
<Highlight color="#0373fc">推薦使用</Highlight> 一次性加載一個主場景與多個子場景。
* **Params**:
...
專門處理從 Prefab 實例化（Clone）出來的小型物件管理，如掉落物、子彈、UI 模板元件 (如物品 ICON 等...)，特性是直接銷毀 (`Destroy`) 即可，會自動連動 AssetLoader 進行卸載。
