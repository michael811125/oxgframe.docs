...
- Preset App Packages。
  - <Highlight color="#00d679">提醒</Highlight> 第一個元素將是「預設包裹 (Default Package)」。
  - ![](/img/assetloader/latest/img_37.png)
- Preset DLC Packages。

![](/img/assetloader/latest/img_8.png)

<Highlight color="#ff2f00">重要</Highlight> Preset Packages 將由**主下載器**進行管理與檢查，所以不包含在 Preset 中的 Packages，必須自行通過 API 初始，說明如下。
...
<Highlight color="#0373fc">注意</Highlight> 另外，[PlayMode](/docs/assetloader/settings#playmode) 的說明如下。
...
<Highlight color="#ff2f00">重要</Highlight> 詳細更新流程，建議參考 [BundleDemo](/docs/samples/#框架模塊範例) (從 Package Manager 匯入)。
