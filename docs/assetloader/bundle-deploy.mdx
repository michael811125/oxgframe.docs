---
sidebar_position: 5
---

# 資源部署

export const Highlight = ({ children, color }) => (
  <span
    style={{
      backgroundColor: color,
      borderRadius: '5px',
      color: '#fff',
      padding: '2.5px'
    }}>
    {children}
  </span>
);

<Highlight color="#ff2f00">重要</Highlight> <Highlight color="#0373fc">注意</Highlight> <Highlight color="#00d679">提醒</Highlight>

### [Coding Style wiki](https://github.com/michael811125/OxGFrame/wiki/Coding-Style)

---

## 內置部署

### 步驟 1.

<Highlight color="#ff2f00">重要</Highlight> 再使用 YooAsset 構建時，直接選擇 Clear And Copy All，將資源包輸出至 StreamingAssets (即內置)。

![](/img/assetloader/img_4.png)

---

## 更新部署

### 步驟 1.

主程式版號配置構建。

通過 MenuItem 點選 OxGFrame -> AssetLoader -> Export Bundle And Config Generator。

![](/img/assetloader/latest/img_9.png)

開啟後，Operation Type 選擇 Export App Config To Streaming Assets，將進行 AppInfo.dat 的輸出 **(用於比對主程式版號)**。

![](/img/assetloader/latest/img_10.png)

### 步驟 2.

<Highlight color="#0373fc">注意</Highlight> 確認 PatchLauncher 上的預設 Packages。

- 預設 App Packages。
- 預設 DLC Packages。

![](/img/assetloader/latest/img_8.png)

### 步驟 3.

通過 MenuItem 點選 OxGFrame -> AssetLoader -> Export Bundle And Config Generator。

![](/img/assetloader/latest/img_9.png)

開啟後，Operation Type 選擇 Export Configs And App Bundles For CDN。

![](/img/assetloader/img_11.png)

1. <Highlight color="#0373fc">注意</Highlight> Source Folder 為 YooAsset 構建輸出的文件夾。
2. <Highlight color="#0373fc">注意</Highlight> 要輸出的 Packages，圖中輸出的是 DefaultPackage。
3. 最後點擊 Process 進行輸出。

另外，可以選擇是否依照完整版號輸出文件夾，可以避免覆蓋的方式上傳至 CDN。

![](/img/assetloader/latest/img_26.png)

### 步驟 4.

將輸出的 CDN 文件夾，直接上傳至雲服務上，完成資源部署。

![](/img/assetloader/img_12.png)

### 步驟 5.

更新資源請求的配置說明。

通過 MenuItem 點選 OxGFrame -> AssetLoader -> Bundle Url Config Generator (CdnConfig.dat)。

![](/img/assetloader/latest/img_23.png)

支持類型為以下 **(不管輸出類型如何，都會自動判別)**：
- Cipher Process (密文類型) -> <Highlight color="#00d679">提醒</Highlight> 建議 Release 發佈使用。
- Plaintext Process (明文類型)。

開啟後，配置以下相關請求訊息：
- CDN Server (Domain name 或者 IP)。
- App 商店的鏈結 (**無則略過**)。


Cipher (密文類型)：

![](/img/assetloader/latest/img_25.png)

Plaintext (明文類型)：

![](/img/assetloader/latest/img_22.png)

---

## 運行包部署

運行包分組定義 (GroupInfo class)。

```csharp
/// <summary>
/// 運行包分組
/// </summary>
[Serializable]
public class GroupInfo
{
    [NonSerialized]
    public int totalCount;
    [NonSerialized]
    public long totalBytes;
    public string groupName;
    public string[] tags;
}
```

YooAsset 收集器中的 Tags 標籤，便於區分運行包 (如有多個 Preset Packages 會自動檢測是否包含配置的 Tags 標籤，會自動整併)。

<Highlight color="#ff2f00">重要</Highlight> 該功能僅限於 [Preset Packages](/docs/assetloader/settings/#preset-packages)，自行獨立初始的 Packages 不適用該功能。

![](/img/assetloader/latest/img_35.png)

區分運行包舉例：
- Minimum_Pack (最小運行包)
- Standard_Pack (標準運行包)

![](/img/assetloader/latest/img_36.png)

<Highlight color="#0373fc">注意</Highlight> groupName 可以用於語系查表，利於多國語系的配置，如 Key: Minimum_Pack, Value: 最小運行包。

<Highlight color="#ff2f00">重要</Highlight> 只有下一次偵測到 App 版號更新時，需要更新 App 時，才會重置上一次使用者選擇群組運行包的紀錄。

---

## 額外說明

<Highlight color="#ff2f00">重要</Highlight> 如果選用以下模式：

- **Offline Mode (離線模式)**
- **WebGL Mode (WebGL 模式)**

只需要進行 [內置部署](#內置部署) 即可。



