...
### Inspector 预览播放器 (v3.7.0+)

`AudioBase` / `VideoBase` 的 Inspector 提供**预览播放器**，无需进入 Play Mode 即可直接试听、试看媒体内容：

- 支持 Play / Pause / Resume / Stop、拖拽进度条与 Timecode 显示，支持全部 Source Type。

- 视频直接在 Inspector 内绘制画面，音轨则在音频线程采样播放。

- 会检测被 Insecure HTTP（Unity 2022.1+）拦截的 `http://` 来源，并询问是否切换为 Only in development builds。
...
如果音讯跟影片来源存放于**云服务**上，可以使用 URL 的方式进行文件请求，格式如下 **(如果不透过 MediaConfig.txt 指定 URL 的话，也可以输入完整资源 URL 至 Prefab 中，不过缺点就是对于未来改动 URL，要进行更改维护就会非常麻烦)**
...
- 通过 MenuItem 点选 OxGFrame -> MediaFrame -> Media Url Config Generator，开始创建 MediaConfig.txt (StreamingAssets/MediaConfig.txt)。

**\>\> 加载 MediaConfig.txt 方式 \<\<**
1. 如果选择 Url Cfg Request Type = Assign，则指定 MediaConfig.txt 至 prefab 中。
2. 如果选择 Url Cfg Request Type = Streaming Assets，将会从 StreamingAssets 进行请求 MediaConfig.txt 文件。
