...
了解全局配置文件 (在 Resources 目录下)。

![](/img/assetloader/img_1.png)

通过右键创建配置文件 (Project 窗体内右键 -> Create -> OxGFrame -> Create Settings -> Create Patch Settings)
...
PatchLauncher 为运行配置，必须拖曳于场景中作为唯一运行配置。
...
- Weak Host Mode (弱联机模式)，首次启动必须联机，以完成初始配置记录与资源下载。
  - <Highlight color="#0373fc">注意</Highlight>：不支持 Skip Download (边玩边下载)，当失去联网时，会验证上一次资源版本的清单内容完整性。
...
[打包专用 Marco](https://github.com/michael811125/OxGFrame/wiki/Macro-Definitions) (发布时，强制切换该模式)：
...
默认包裹配置 App Packages & DLC Packages，将由 AssetPatcher.Check 主更新阶段进行更新。
