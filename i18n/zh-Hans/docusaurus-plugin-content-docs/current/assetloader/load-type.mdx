...
- Preset App Packages。
  - <Highlight color="#00d679">提醒</Highlight> 第一个元素将是「预设包裹 (Default Package)」。
  - ![](/img/assetloader/latest/img_37.png)
- Preset DLC Packages。

![](/img/assetloader/latest/img_8.png)

<Highlight color="#ff2f00">重要</Highlight> Preset Packages 将由**主下载器**进行管理与检查，所以不包含在 Preset 中的 Packages，必须自行通过 API 初始，说明如下。
...
<Highlight color="#0373fc">注意</Highlight> 另外，[PlayMode](/docs/assetloader/settings#playmode) 的说明如下。
...
<Highlight color="#ff2f00">重要</Highlight> 详细更新流程，建议参考 [BundleDemo](/docs/samples/#框架模块示例) (从 Package Manager 中汇入)。
