...
- OnPreShow : 当有异步处理或者附属物件控制时，可以在此处理。例如 : TopUI 附属联动开启 LeftUI & RightUI，那么就可以在 TopUI 中的 OnPreShow 方法实现 Show LeftUI & RightUI。
  - <Highlight color="#00d679">提醒</Highlight> **不建议在 OnPreClose 时进行相关 Show 的处理，如果有进行的话也没关系，因为针对 CloseAll 的 API 有提供 disableOnPreClose 参数开关。**
...
- UIBase & SRBase
  - _Node@XXX\*Btn (以此类推)
  - _Node@XXX\*Btn\*Img\*Trans (以此类推，支持多个 \* 指向多 TailName)
- CPBase
  - ~Node@XXX\*Btn (以此类推)
  - ~Node@XXX\*Btn\*Img\*Trans (以此类推，支持多个 \* 指向多 TailName)
...
默认组件绑定表 **(可从 CodeBindingSettings 中自行新增或修改 TailName 对应绑定组件类型)**
