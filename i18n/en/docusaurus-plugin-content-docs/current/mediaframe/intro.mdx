...
### Inspector Preview Player (v3.7.0+)

The `AudioBase` / `VideoBase` inspectors provide a **preview player**, letting you audition media directly without entering Play Mode:

- Play / Pause / Resume / Stop with a seek bar and timecode display; all Source Types are supported.

- Video renders directly inside the inspector, while audio is sampled and played on the audio thread.

- Detects `http://` sources blocked by Insecure HTTP (Unity 2022.1+) and offers to switch to "Only in development builds".
