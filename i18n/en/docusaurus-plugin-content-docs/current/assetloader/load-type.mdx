...
- Preset App Packages。
  - <Highlight color="#00d679">Reminder</Highlight> The first element will be the Default Package.
  - ![](/img/assetloader/latest/img_37.png)
- Preset DLC Packages.

![](/img/assetloader/latest/img_8.png)

<Highlight color="#ff2f00">Important</Highlight> The Preset Packages will be managed and checked by the **main downloader**, so Packages not included in the Presets must be initialized manually through the API, as described below.
...
<Highlight color="#0373fc">Attention</Highlight> Additionally, the [PlayMode](/docs/assetloader/settings#playmode) descriptions are as follows:
...
<Highlight color="#ff2f00">Important</Highlight> For detailed update procedures, it is recommended to refer to [BundleDemo](/docs/samples/#framework-module-samples) (import via Package Manager).
