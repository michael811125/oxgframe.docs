...
<Highlight color="#0373fc">Attention</Highlight> Additionally, the [Play Mode](/docs/assetloader/settings#playmode) descriptions are as follows:
- Editor Simulate Mode (Development Mode).
- Offline Mode (Local Mode).
- Host Mode (Online Mode).
- Weak Host Mode (Lite Online Mode).
- WebGL Mode (Requesting from StreamingAssets).
- WebGL Remote Mode (Requesting from StreamingAssets and other CDNs).
